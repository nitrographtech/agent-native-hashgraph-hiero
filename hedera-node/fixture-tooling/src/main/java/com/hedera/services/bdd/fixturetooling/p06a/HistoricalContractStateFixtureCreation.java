...
import com.hedera.services.bdd.suites.HapiSuite.FinalOutcome;
...
public final class HistoricalContractStateFixtureCreation extends HapiSuite {
    ...
    public static void main(final String... args) {
        final var suite = new HistoricalContractStateFixtureCreation();
        final var outcome = suite.runSuiteSync();
        if (outcome != FinalOutcome.SUITE_PASSED) {
            suite.getFinalSpecs().stream()
                    .map(HapiSpec::getCause)
                    .filter(java.util.Objects::nonNull)
                    .forEach(failure -> failure.cause().printStackTrace(System.err));
            throw new IllegalStateException("Historical contract fixture generation failed: " + outcome);
        }
    }
    ...
    Stream<DynamicTest> populatesV065Maps() {
        ...
        return hapiTest(
                overriding("contracts.sidecars", "CONTRACT_ACTION,CONTRACT_STATE_CHANGE,CONTRACT_BYTECODE"),

...
public final class P06aPublicFixtureIdentity {
    ...
    private static final long EXPECTED_NODE_ID = 0L;
    ...
    public static void main(final String... args) throws Exception {
        ...
        if (!WORKFLOW_VALUE.equals(System.getenv(WORKFLOW_ENV))) {
            throw new IllegalArgumentException(
                    WORKFLOW_ENV + "=" + WORKFLOW_VALUE + " is required");
        }
        final long networkId = parseExact(options, "--network-id", EXPECTED_NETWORK_ID);
        final long nodeId = parseExact(options, "--node-id", EXPECTED_NODE_ID);
        ...
        final Path privatePem = output.resolve("s-private-node1.pem");
        final Path publicPem = output.resolve("s-public-node1.pem");
    ...
    private static SecureRandom deterministicRandom(
            final String purpose, final long networkId, final long nodeId) throws Exception {
    ...
    private static long parseExact(
            final Map<String, String> options, final String option, final long expected) {

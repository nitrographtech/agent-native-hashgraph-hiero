...
| Execution tracers | contract implementation | `impl/exec/tracers/**`; `ActionSidecarContentTracer`; action stack utilities | tracer/action-sidecar tests; pinned full-node fixture generation | Native shared translation is neutral. Live PBJ actions are produced only by `EvmActionTracer`/`ActionStack`; the authenticated fixture's three-action corpus requires this producer. | EXECUTION_TRACER, FIXTURE_TOOLING_REQUIRED | BLOCKED: choose fixture-only producer ownership or revise fixture reproducibility before deletion |

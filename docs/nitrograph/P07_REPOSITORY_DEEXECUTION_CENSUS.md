...
| Standalone executor | `hedera-app`; same JPMS module | `workflows/standalone/**`, plus `fees/StandaloneFeeCalculatorImpl` | 3 standalone tests plus fee/record tests | Native app jar: absent. Standalone: direct `ContractServiceImpl` and tracer construction. | STANDALONE_ONLY | Medium: shares the app module and fee tests |
...
| Execution tracers | contract implementation | `impl/exec/tracers/**`; `ActionSidecarContentTracer`; action stack utilities | tracer/action-sidecar tests; standalone tracer binding | Native shared translation is neutral. Producers exist only in full/standalone execution. | EXECUTION_TRACER | Medium after standalone removal |
...
| Fixture identity tooling | `test-clients` | `fixtures/p06a/P06aPublicFixtureIdentity`, Gradle generator task | P06A reproducible public test identity | Identity derivation itself is non-secret; executable node generation depends on full runtime. | FIXTURE_ONLY; descriptor is PERMANENT evidence | Medium: separate identity from executable transaction suites |
| Historical fixture generation suites | `test-clients` | `DiverseStateCreation`, `HistoricalContractStateFixtureCreation`, freeze and legacy Ethereum suites | pinned full-runtime generation workflow | Not native runtime. Needed only to regenerate authentic pre-activation evidence. | FIXTURE_ONLY | High: freeze a final externalized generator before removal |

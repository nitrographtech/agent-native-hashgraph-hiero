...
| 3 | Execution tracers | **PREREQUISITE COMPLETE.** P07-3A moved the minimum live `EvmActionTracer`/`ActionStack` closure to `fixture-tooling`; normal native and full runtime artifacts contain no fixture tracer. Retain PBJ action/state/bytecode models and neutral translators. | Resume deletion of remaining runtime tracer producers and bindings while preserving the isolated fixture producer. | Full-runtime output delta and historical mirror gates remain mandatory |

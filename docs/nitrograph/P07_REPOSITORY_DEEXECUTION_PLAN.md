...
| 6 | Ethereum execution | **IN PROGRESS (P07-6).** Remove the live handler, hydration, signature cache, fee/gas branches, dispatch, and runtime parser ownership. Retain test-client corpus helpers until their mixed-suite consumers are retired. | Retain Ethereum PBJ bodies/results for decoding, rejection, and mirror | High. Historical Ethereum mirror counts and five-body rejection must pass |
...
official mirror database ingestion is still required because the validation host denied access to
its active Docker daemon. Wave 6 must not be marked complete or merged until that environmental
gate is rerun successfully.

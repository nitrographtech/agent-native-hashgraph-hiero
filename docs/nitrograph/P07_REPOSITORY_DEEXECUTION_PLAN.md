...
| 3 | Execution tracers | **BLOCKED AFTER P07-3A.** The concrete producer is fixture-owned, but `FrameRunner`, `CustomMessageCallProcessor`, HEVM, Bonneville, and result construction still own its Hedera-specific callback protocol. Retain PBJ action/state/bytecode models and neutral translators. | Requires a separately authorized fixture execution callback adapter, or must follow system-contract/EVM processor removal. | Deleting the protocol now crosses into Wave 4/system-contract and world-state execution |

...
| 10 | `app-service-contract-impl` module | **P07-9 COMPLETE.** Legacy-FQN schema forwarders moved unchanged to the neutral owner; conversion residue moved to test clients; dead opcode utility and project shell deleted. Exact-head CI, both authenticated fixture boundaries, save/restart, real four-node reconnect, protected maps, and the three-corpus official mirror regression passed. | `RecordStreamBuilder`, six builder interfaces, historical schemas/stores remain in neutral API | Merge after the final evidence-only head passes CI |
